...
이 장도 다른 장과 같이 두 트랙으로 나뉜다. 기업 편은 정책·라이선스·규제를 다루고, 개발자 편은 AI 코딩 도구 사용과 모델 반입 실무를 다룬다. 같은 주제를 두 트랙에 중복해 서술하지 않으므로, 정책을 세우는 독자는 기업 편을, 커밋과 배포를 직접 하는 독자는 개발자 편을 먼저 읽는다.
...
|---|---|---|---|
| 반입 승인 | 컴포넌트 단위 라이선스 확인과 승인 요청 | 모델 라이선스·AUP<sub>Acceptable Use Policy</sub>·지역 제한 확인 | [모델 라이선스 실사](#모델-라이선스-실사), [「오픈소스 사용하기」](/using/) |
| 대장 등록 | 컴포넌트명·버전·라이선스 기록 | 개방성 등급, 베이스 모델 계보, 데이터셋 참조를 추가 기록 | [사내 표기 규칙 — 모델 대장의 '개방성 등급' 컬럼](#사내-표기-규칙--모델-대장의-개방성-등급-컬럼) |
| SBOM | SPDX·CycloneDX 컴포넌트 항목으로 표현 | SPDX AIPackage 또는 CycloneDX modelCard로 표현 | [「오픈소스 사용하기」](/using/)의 SBOM 관리 절 |
| 고지 | NOTICE 파일과 라이선스 전문 고지 | NOTICE 문구, 'Built with Llama' 류 표시, AI 기본법 고지를 함께 처리 | [한국 AI 기본법의 고지·표시 의무 — 오픈소스 모델도 면제되지 않는다](#한국-ai-기본법의-고지표시-의무--오픈소스-모델도-면제되지-않는다) |
| 배포 승인 | 배포 형태별 의무 이행 확인 | 파생 모델 명명 규칙, 라이선스 사본 동봉, AUP 전가 확인 | [파인튜닝 결과물의 라이선스 승계 — 5개 항목 배포 승인 체크리스트](#파인튜닝-결과물의-라이선스-승계--5개-항목-배포-승인-체크리스트), [「오픈소스 공개하기」](/releasing/) |

기존 컴포넌트 관리 대장에 '자산 유형' 컬럼(라이브러리 / 모델 / 데이터셋)을 추가하고 유형별 필수 필드를 다르게 정의하면 위 표가 그대로 대장 스키마가 된다. 조직 역할과 승인 권한을 어디에 둘지는 [「OSPO」](/ospo/)에서 다룬다.
...
이 절의 목적은 사내 문서·조달 문서·제품 고지에서 어떤 모델을 '오픈소스'로 부를 수 있는지 판정하는 것이다.
...
OSAID를 확정된 표준처럼 인용하면 사내 정책이 한쪽 입장에만 서게 된다. 세 진영의 비판이 남아 있다. Software Freedom Conservancy의 Bradley Kuhn은 2024-10-31 블로그에서 'the OSAID fails to place sufficient requirements on the licensing and public disclosure of training sets'라고 비판했다[^ai-sfc-osaid]. FSF는 별도로 'free machine learning application' 기준을 준비 중이며 학습 데이터와 그 처리 스크립트까지 자유 소프트웨어여야 한다는 더 엄격한 입장이다. Debian 진영은 OSAID-1.0-RC2 단계에서 데이터 정보만 요구하면 오픈소스 AI가 원본 학습 데이터를 감출 수 있게 된다는 반대 의견을 메일링 리스트로 제출했다[^ai-debian-osaid].
...
|---|---|---|---|
| 데이터 공개 오픈소스 AI | 배포 코드와 학습 코드, 경우에 따라 전체 학습 데이터까지 오픈소스SW 라이선스로 무상 공개 | 없음 | (근거 문서에 분류 사례 없음) |
| 오픈소스 AI | 위와 같으나, 학습 데이터 전체 공개가 법적·기술적으로 불가능한 경우 그 데이터에 관한 데이터 정보로 대체 | 없음 | (근거 문서에 분류 사례 없음) |
...
앞의 두 기준을 사내 운영으로 내리는 방법은 모델 등록 대장에 '개방성 등급' 컬럼을 필수 항목으로 두는 것이다. 값은 자유 입력을 막고 다음 다섯 개로 고정한다. G7 4단계를 그대로 채택하는 이유는 그것이 국제 준거이기 때문이며, 벤더 API만 사용하는 경우를 담기 위해 '비공개' 하나를 더한다.
...
이 컬럼을 강제하는 이유는 두 가지 실무 위험 때문이다. 첫째, 조달 문서에 '오픈소스 AI 사용'으로 기재해 발주처의 오픈소스SW 정책 심사를 통과했으나 실제로는 사용 제한이 있는 커스텀 라이선스인 경우가 있다. 둘째, 같은 오인이 사내 오픈소스SW 승인 프로세스를 우회하는 통로가 된다. '라이선스 미표기'가 곧 '오픈소스'로 읽히기 쉬운 배경에는 Hugging Face 모델의 약 70%가 라이선스 표기 자체를 갖고 있지 않다는 실태가 있다[^ai-redmonk-hf]. 상세 수치와 개발자 단계의 대응은 [라이선스 미표기는 '오픈소스'가 아니라 '모든 권리 유보'다 — Hugging Face 실태](#라이선스-미표기는-오픈소스가-아니라-모든-권리-유보다--hugging-face-실태)에서 다룬다.
...
Gemma 4는 2026-04-02 Apache-2.0으로 공개되어 'Gemma 4 models are the first in the Gemmaverse to be released under the OSI-approved Apache 2.0 license'로 발표됐다[^ai-gemma4]. 반면 Gemma 3 이하에는 커스텀 Gemma Terms of Use(최종 수정 2026-04-01)와 Prohibited Use Policy가 그대로 적용된다[^ai-gemma-terms]. 구세대에 남는 의무는 다음 네 가지다.
...
[^ai-gemma4]: Google Open Source Blog, Gemma 4: Expanding the Gemmaverse with Apache 2.0 : https://opensource.googleblog.com/2026/03/gemma-4-expanding-the-gemmaverse-with-apache-20.html
...
- 'You shall not supply the Mistral Models, Derivatives, or Outputs in the course of a commercial activity, whether in return for payment or free of charge, in any medium or form, including but not limited to through a hosted or managed service (e.g. SaaS, cloud instances, etc.), or behind a software layer.'
...
<!-- NEEDS-EVIDENCE: MNPL 원문의 정본 URL 확정 — mistral.ai/licenses/MNPL-0.1.md 와 별도 경로가 함께 존재해 어느 쪽이 정본인지 확인 필요 -->
...
| 사용 제한이 반드시 필요하다 | 커스텀 라이선스 | 법무 승인 필수. EU AI Act 오픈소스 예외를 받지 못한다 |

OpenMDW는 Linux Foundation이 2026-05-28 OpenMDW-1.1을 공개했고, NVIDIA가 Cosmos·Isaac GR00T·Ising·Nemotron 모델군에 채택했다고 발표했다[^ai-openmdw-lf]. SPDX License List 3.28.0에는 OpenMDW-1.0 식별자가 등재되어 있다[^ai-spdx-list]. 다만 OSI 승인 여부는 공식 페이지에 언급이 없어 확인되지 않는다 — 'OSI 승인 라이선스다'라고 표기하지 않는다[^ai-openmdw].

커스텀 라이선스를 선택하면 [EU AI Act의 오픈소스 예외는 좁다 — 무엇이 예외를 깨는가](#eu-ai-act의-오픈소스-예외는-좁다--무엇이-예외를-깨는가)에서 다루는 예외가 깨진다. 공개 절차 자체(공개 심의, 저장소 준비, 커뮤니티 운영)는 [「오픈소스 공개하기」](/releasing/)에서 다룬다.

<!-- NEEDS-EVIDENCE: OpenMDW-1.1의 OSI 승인 여부 — 공식 페이지에 언급이 없어 미확인. OSI 라이선스 목록 등재 여부 재확인 필요 -->
...
OpenRAIL 계열 라이선스는 상업적 사용을 허용하면서도 감시·추적, 허위정보 생성, 차별 등의 용도를 금지하고, 그 제한을 파생물에도 같거나 더 엄격한 형태로 부착할 것을 요구한다[^ai-openrail]. 사용 제한이 있으므로 OSI 승인 라이선스가 아니다. OSI는 'Should OpenRAIL licenses be considered OS AI Licenses?' 웨비나를 열어 이 문제를 논의했으나 승인하지 않았다[^ai-openrail-osi]. Hugging Face 상 RAIL 계열은 모델 약 5만 개, 데이터셋 약 5천 개 규모로 보고된다.
...
<!-- NEEDS-EVIDENCE: RAIL 계열 모델 약 5만 개·데이터셋 약 5천 개 규모의 1차 집계 출처(Hugging Face 태그 기준 집계일 포함) -->
...
서술 강도에 주의한다. 이들 라이선스의 강제력은 저작권이 아니라 계약(약관 수락)에 근거한다는 해석이 유력하고, 미국 저작권청이 순수 AI 생성물의 저작물성을 부정하는 입장이어서 가중치의 저작물성 자체가 다투어질 수 있다[^ai-uscopyright]. 따라서 '저작권 침해가 된다'로 단정하지 않고 '계약 위반 리스크가 실재한다' 수준으로 판단한다. 개발자가 배포 직전에 손으로 확인할 항목은 [파인튜닝 결과물을 배포하기 전에 확인할 것](#파인튜닝-결과물을-배포하기-전에-확인할-것)에 별도로 두었다.
...
EU 시장에 모델을 출하하는 경우 학습 콘텐츠 공개 요약 의무와 직결되므로 [EU AI Act의 오픈소스 예외는 좁다 — 무엇이 예외를 깨는가](#eu-ai-act의-오픈소스-예외는-좁다--무엇이-예외를-깨는가)를 함께 확인한다.
...
[^ai-bigcode-stack]: BigCode, The Stack — BigCode Datasets : https://www.bigcode-project.org/docs/about/the-stack/
...
집행위의 2025-07-18 'Guidelines on the scope of obligations for general-purpose AI models'는 GPAI 오픈소스 예외의 범위를 아래와 같이 해석한다[^ai-eu-gpai-guidelines]. 왼쪽에 해당하는 조건이 하나라도 있으면 예외를 주장할 수 없다.
...
세 가지를 함께 확인한다. 첫째, 체계적 위험(systemic risk) GPAI는 오픈소스여도 예외 없이 전 의무를 진다. 둘째, 예외가 적용되어도 학습 콘텐츠 공개 요약과 EU 저작권법 준수 정책 의무는 남는다[^ai-eu-article53]. 셋째, 그 직접 결과로 700M MAU 임계를 둔 Llama 4는 이 예외를 받지 못한다. 시행 일정은 2025-08-02 GPAI 의무 적용 개시, 2026-08-02 AI Office 집행 개시, 2027-08-02 기존 모델 소급 준수 기한이다[^ai-lfe-aiact].
...
[^ai-lfe-aiact]: Linux Foundation Europe, What Open Source Developers Need to Know about the EU AI Act (2차 출처) : https://linuxfoundation.eu/newsroom/ai-act-explainer
...
실행 지시는 하나다. AI 고지·표시를 기존 오픈소스SW 고지 화면과 같은 위치에 배치해 운영 부담을 줄이고, 오픈소스 모델 사용이 면제 사유가 아님을 정책 문서에 명시한다.
...
한국 저작권법에는 이에 대응하는 유권해석이 별도로 정리되어 있지 않아 국내 적용은 미확립이다. 실무에서는 릴리스 검토 항목 하나로 다룬다 — AI 생성 비중이 높은 산출물을 오픈소스SW로 사외 공개할 때 저작권이 성립하지 않아 라이선스 부여가 무의미해질 수 있다는 점을 검토하고, 인간 기여 기록(리뷰·수정 이력)을 남긴다. 공개 절차는 [「오픈소스 공개하기」](/releasing/)에서 다룬다.

<!-- NEEDS-EVIDENCE: AI 생성물의 저작물성에 관한 국내 유권해석·판례·문화체육관광부 지침 여부 -->
...
<!-- NEEDS-EVIDENCE: Doe v. GitHub 제9순회 사건번호 확정 — 자료별로 24-6136과 24-7700이 혼재해 본문에 기재하지 않음 -->
...
Microsoft Learn은 GitHub Offerings에 대해 'as of April 3, 2026, there are no additional required mitigations. Use of the Duplicate Detection filter feature is no longer required for CCC coverage. This feature remains available for optional use.'라고 밝힌다. 반면 다른 제품에는 요구가 남아 있다 — Azure OpenAI 코드 생성 시나리오는 protected material code model을 annotate 또는 filter 모드로 설정해야 하고 annotate 모드를 택하면 출력물에 인용된 라이선스를 준수해야 하며(2023-12-01 발효), 비동기 필터 사용 시 사후 플래그된 출력물은 인용 라이선스를 준수하지 않으면 CCC가 적용되지 않는다(2024-05-21 발효). Copilot Studio는 외부 호스팅 모델을 연결하면 CCC가 적용되지 않는다(2025-06-01 발효)[^ai-ccc].
...
2026년 기준 스니펫 매칭의 대표 사례로는 FOSSA가 2026-01 SCANOSS와 기술 제휴를 맺어 SCANOSS의 스니펫 탐지와 FOSSA의 라이선스 지식베이스를 결합한 것[^ai-fossa-scanoss], Black Duck이 Snippet Analysis를 API로 제공해 AI 생성 코드를 대량 분석할 수 있게 한 것[^ai-blackduck-snippet]이 있다. 제품 간 우열이나 가격은 이 가이드에서 비교하지 않는다.
...
이 3중 검사는 릴리스 절차 안에 놓이는 게이트이며, 릴리스 게이트 자체의 구성과 승인 흐름은 [「오픈소스 공개하기」](/releasing/)에서 다룬다. AI가 존재하지 않는 패키지명을 만들어 내는 패키지 환각과 그것을 노린 슬롭스쿼팅은 공급망 보안 주제이므로 이 장에서 다루지 않는다 — [「오픈소스 사용하기」](/using/)의 보안 절을 참조한다.
...
라이선스 미표기는 오픈소스가 아니라 '모든 권리 유보'다. 미표기 모델의 사용 금지를 기본 정책으로 하고, 예외로 사용해야 할 때는 법무 검토를 거친다.
